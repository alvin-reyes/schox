	...
	function listAllData($page = 1){
		...
		if($this->data['users']->role == "parent" ){
			...
			if(count($studentId) > 0){
				$toReturn['students'] = User::where('role','student')->whereIn('id', $studentId)->orderBy('studentRollId','ASC')->take('20')->skip(20* ($page - 1) )->get()->toArray();
			}else{
				$toReturn['students'] = array();
			}
		}else{
			$toReturn['students'] = User::where('role','student')->orderBy('studentRollId','ASC')->take('20')->skip(20* ($page - 1) )->get()->toArray();
		}
